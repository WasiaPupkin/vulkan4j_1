package tutorial.vulkan.raytracing.ch23;
 ...
 *
 * <p>Renders the same rotating colored quad as the graphics pipeline in
 * part07/ch23/Main.java, but using ray tracing instead of rasterization.
 * The quad has 4 vertices with interpolated vertex colors (red, green, blue, white)
 * and rotates 90 degrees per second around the Z axis, viewed from (2,2,2).</p>
 */
public class Application {
    ...
    private static final BytePtr WINDOW_TITLE = BytePtr.allocateString(Arena.global(), "Chapter 23 - Ray Traced Rotating Quad");
    ...
    // ======================== Vertex & Geometry ========================
    private static final int VERTEX_STRIDE_BYTES = 20; // 5 floats * 4 bytes (pos: 2 + color: 3)
    ...
    private void createVertexBuffer() {
        // 4 vertices forming a quad (same as part07/ch23 Main.java)
        // vec2 pos + vec3 color
        float[] vertices = {
                // Position    // Color
                -0.5f, -0.5f,  1.0f, 0.0f, 0.0f,   // vertex 0: red,    bottom-left
                 0.5f, -0.5f,  0.0f, 1.0f, 0.0f,   // vertex 1: green,  bottom-right
                 0.5f,  0.5f,  0.0f, 0.0f, 1.0f,   // vertex 2: blue,   top-right
                -0.5f,  0.5f,  1.0f, 1.0f, 1.0f,   // vertex 3: white,  top-left
        };
        try (var arena = Arena.ofConfined()) {
    ...
    private VkDescriptorPoolSize.Ptr populateDescriptorPoolSizes(Arena arena) {
        return VkDescriptorPoolSize.allocate(arena, 2)
    ...
    private void createRayTracingPipeline() {
        try (var arena = Arena.ofConfined()) {
            var rgenCode = compileShader(arena, "/shader/raytracing/ch23/ray.rgen", "rgen");
            var rchitCode = compileShader(arena, "/shader/raytracing/ch23/ray.rchit", "rchit");
            var rmissCode = compileShader(arena, "/shader/raytracing/ch23/ray.rmiss", "rmiss");
