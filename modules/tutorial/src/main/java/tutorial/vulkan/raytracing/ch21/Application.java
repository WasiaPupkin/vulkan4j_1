package tutorial.vulkan.raytracing.ch21;
...
/**
 * Chapter 21 — Ray Tracing: Textured Quad with Checkerboard Pattern.
 *
 * <p>Renders a textured quad with a checkerboard pattern using ray tracing,
 * building upon the gradient triangle from Chapter 20.</p>
 */
public class Application {
    ...
    private static final BytePtr WINDOW_TITLE = BytePtr.allocateString(Arena.global(), "Chapter 21 - Ray Traced Textured Quad");
    ...
    // ======================== Vertex & Geometry ========================
    private static final int VERTEX_STRIDE_BYTES = 24; // 6 floats * 4 bytes (pos: 3 + color: 3)
    ...
    private void createInstance() {
        try (var arena = Arena.ofConfined()) {
            ...
            var appInfo = VkApplicationInfo.allocate(arena)
                    .pApplicationName(BytePtr.allocateString(arena, "Ch21 Ray Tracing"))
    ...
    private void createVertexBuffer() {
        // 4 vertices forming a quad (same as part06/ch21 Main.java, scaled for RT view)
        float[] vertices = {
                // Position         // Color
    ...
    private void createIndexBuffer() {
        ...
        try (var arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < indices.length; i++) {
                seg.set(ValueLayout.JAVA_INT, i * Integer.BYTES, indices[i]);
            }
    ...
    private long createAndBuildBlas(Arena arena, long vertexAddress, long indexAddress, int alignment) {
        ...
        for (int i = 0; i < transform.length; i++) {
            transformSeg.set(ValueLayout.JAVA_FLOAT, i * Float.BYTES, transform[i]);
        }
    ...
    private void createRayTracingPipeline() {
        try (var arena = Arena.ofConfined()) {
            var rgenCode = compileShader(arena, "/shader/raytracing/ch21/ray.rgen", "rgen");
            var rchitCode = compileShader(arena, "/shader/raytracing/ch21/ray.rchit", "rchit");
            var rmissCode = compileShader(arena, "/shader/raytracing/ch21/ray.rmiss", "rmiss");
